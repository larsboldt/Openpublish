 ...
 */
class opJPGFile extends opGraphicsFile {
    private $width;
    private $height;
    ...
    protected function generateThumbnail($maxsize, $destPath) {
        return $this->resizeImage($this->getWidth(), $this->getHeight(), $maxsize, $this->getBasePath().$this->getFilename(), $destPath, 100);
    }

    private function resizeImage($originalWidth, $originalHeight, $newMaxSize, $sourcePath, $destPath, $quality) {
        if (is_writable($destPath)) {
            $imRS = imagecreatefromjpeg($sourcePath);
            if ($imRS) {
                ...
                imagejpeg($imRS_resized, $destPath.$newMaxSize.'_'.$this->getFilename(), $quality);
